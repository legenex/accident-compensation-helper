  ...
  return (
    <button
      ...
      className={`group relative flex flex-col items-start rounded-2xl border p-6 text-left transition-all ${
        selected
          ? "border-primary bg-primary/[0.06] shadow-float"
          : "border-border bg-card hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lift"
      }`}
    >
...
export default function ClaimCheck() {
  ...
  const steps = useMemo(
    () => [
      ...
      { key: "at_fault", title: "Who was at fault?", sub: "This helps us understand the strength of your case." },
      { key: "sought_treatment", title: "Did you seek medical treatment?", sub: "Treatment records strengthen most claims." },
      { key: "injury_tier", title: "How would you describe your injuries?", sub: "Choose the tier that best fits your recovery." },
      { key: "damages", title: "Your medical bills and lost wages", sub: "Estimates are fine. We use these to value your claim." },
      { key: "location", title: "Where did it happen?", sub: "State and ZIP help us match you with a local attorney." },
      { key: "contact", title: "Where can we reach you?", sub: "We'll send your free results and connect you with an attorney." },
    ],
    []
  );
  ...
  const canProceed = () => {
    ...
    if (s === "contact")
      return answers.first_name && answers.last_name && answers.email && answers.phone;
    return answers[s] !== "";
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        sought_treatment: answers.sought_treatment,
        injury_tier: answers.injury_tier,
        medical_bills: Number(answers.medical_bills) || 0,
        lost_wages: Number(answers.lost_wages) || 0,
        economic_damages: estimate.economic_damages,
        non_econ_low: estimate.non_econ_low,
        non_econ_high: estimate.non_econ_high,
        range_low: estimate.range_low,
        range_high: estimate.range_high,
        represented_low: estimate.represented_low,
        represented_high: estimate.represented_high,
        is_opted_in: true,
      ...
      const lead = await base44.entities.Lead.create({
        ...
        estimate_low: estimate.range_low,
        estimate_high: estimate.range_high,
    ...
    <div className="min-h-screen bg-gradient-to-b from-secondary/40 via-background to-background pt-16">
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-3xl flex-col justify-center px-6 py-16">
        ...
        <div key={step} className={slideClass}>
          ...
          <h1 className="mx-auto mt-4 max-w-2xl text-center font-heading text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">
            {steps[step].title}
          </h1>
          ...
          <div className="mx-auto mt-10 max-w-2xl">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {ACCIDENT_TYPES.map((t) => (
                  <ChoiceCard
                    ...
                    title={t.label}
                    desc={t.blurb}
                    selected={answers.accident_type === t.id}
              ...
              <div className="mx-auto max-w-sm">
                ...
                {answers.accident_date && !withinLastYears(answers.accident_date, 2) && (
                  <p className="mt-3 rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-700">
                    The incident may be outside the typical filing window. You can still continue and an attorney can
            ...
            {s === "at_fault" && (
              <div className="grid gap-4 sm:grid-cols-2">
                {FAULT.map((f) => (
                  <ChoiceCard key={f.id} title={f.label} desc={f.desc} selected={answers.at_fault === f.id} onClick={() => setField("at_fault", f.id)} />
                ))}
              </div>
            )}
            ...
            {s === "sought_treatment" && (
              <div className="grid gap-4 sm:grid-cols-2">
                {TREATMENT.map((f) => (
                  <ChoiceCard key={f.id} title={f.label} desc={f.desc} selected={answers.sought_treatment === f.id} onClick={() => setField("sought_treatment", f.id)} />
                ))}
              </div>
            )}
            ...
            {s === "injury_tier" && (
              <div className="grid gap-4 sm:grid-cols-2">
                {INJURY_TIERS.map((f) => (
                  <ChoiceCard key={f.id} title={f.label} desc={f.desc} selected={answers.injury_tier === f.id} onClick={() => setField("injury_tier", f.id)} />
                ))}
              </div>
            )}
                ...
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setField("medical_bills", e.target.value)}
                      className="h-14 rounded-xl pl-8 text-lg"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input
                      ...
                      inputMode="numeric"
                      placeholder="0"
                      value={answers.lost_wages}
                      onChange={(e) => setField("lost_wages", e.target.value)}
                      className="h-14 rounded-xl pl-8 text-lg"
                    />
                  </div>
                </div>
              ...
              <div className="mx-auto grid max-w-md gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <select
                    ...
                    className="mt-2 h-14 w-full rounded-xl border border-input bg-card px-4 text-lg text-foreground outline-none focus:ring-2 focus:ring-primary"
                  >
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setField("zip_code", e.target.value.replace(/\D/g, ""))}
                    className="mt-2 h-14 rounded-xl text-lg"
                  />
                </div>
              </div>
            ...
            {s === "contact" && (
              <div className="mx-auto grid max-w-md gap-4">
                ...
                <p className="text-xs text-muted-foreground">
                  By continuing, you consent to be contacted about your claim. This is a free service with no obligation.
                </p>
              </div>
            )}
        ...
        <div className="mx-auto mt-12 flex w-full max-w-2xl items-center justify-between">
          <Button
            ...
            onClick={back}
            disabled={step === 0}
            className="gap-2 rounded-full px-5 text-muted-foreground disabled:opacity-0"
          >
          ...
          {step < steps.length - 1 ? (
            <Button
              onClick={next}
              disabled={!canProceed()}
              className="gap-2 rounded-full bg-primary px-8 py-6 text-base font-semibold shadow-float hover:bg-primary/90 disabled:opacity-40"
            >
              Proceed
